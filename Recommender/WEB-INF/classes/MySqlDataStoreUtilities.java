...
public  static void readOutputFile(){
	...
	try {
		...
		br = new BufferedReader(new FileReader(new File(TOMCAT_HOME+"\\webapps\\Recommender\\output.csv")));
		while ((line = br.readLine()) != null) {
			...
			String user =prod_recm[0];
			String recommends =prod_recm[1];
			...
			String[] arrOfStr = recommends.split(",");
        for (String a: arrOfStr){
		insertToRecommendationTable(a.trim(),user);
		}
...
public static void insertToRecommendationTable(String id, String user){
	
	try{


		getConnection();
		String selectPlace = "select * from  recommend where id=?";
		PreparedStatement pst = conn.prepareStatement(selectPlace);
		pst.setString(1,id);
	    ...
	    if(rs.next()){
			...
			String insertPlacesQuery = "INSERT INTO  recommendation(id,name,image,address,rating,user)" + "VALUES (?,?,?,?,?,?);";

	 	PreparedStatement pst2 = conn.prepareStatement(insertPlacesQuery);
		 pst2.setString(1,rs.getString("Id"));
		  	pst2.setString(2,rs.getString("name"));
		 	pst2.setString(3,rs.getString("image"));
		 	pst2.setString(4,rs.getString("address"));
			pst2.setFloat(5,rs.getFloat("rating"));
...
public static void insertToTable(String id){
	
	try{

		getConnection();
		String selectPlace = "select * from  Places where id=?";
		PreparedStatement pst = conn.prepareStatement(selectPlace);
		pst.setString(1,id);
		ResultSet rs = pst.executeQuery();
		...
		if(rs.next()){
			...
			String insertPlacesQuery = "INSERT INTO  recommend(id,name,image,address,rating)" + "VALUES (?,?,?,?,?);";
...
public static void DeletePlaces(){
	try{
	 ...
	 String truncatetableprod = "delete from  Places;";
...
public static void InsertPlaces(String id, String name, String address, String image, float rating){

	try{
		getConnection();
		String insertPlacesQuery = "INSERT INTO  Places(id,name,image,address,rating)" + "VALUES (?,?,?,?,?);";

	 	PreparedStatement pst = conn.prepareStatement(insertPlacesQuery);
		 pst.setString(1,id);
...
{	
	...
	try 
	{
		...
		while(rs.next()){
			...
			Place pl = new Place(rs.getString("id"),rs.getString("name"),rs.getString("image"),rs.getString("address"),rs.getFloat("rating"));
				hm.put(rs.getString("id"), pl);
...
public static HashMap<String,Place> getPlacesForUser(String user)
{	
	HashMap<String,Place> hm=new HashMap<String,Place>();
	try 
	{
		...
		String selectPlace="select * from  recommendation where user=?";
		PreparedStatement pst = conn.prepareStatement(selectPlace);
...
public static HashMap<String,Place> getData()
	{
		HashMap<String,Place> hm=new HashMap<String,Place>();
		try
		{
			getConnection();
			Statement stmt=conn.createStatement();
			String selectCustomerQuery="select * from  recommendation";
			ResultSet rs = stmt.executeQuery(selectCustomerQuery);
			while(rs.next())

			{	Place p = new Place(rs.getString("id"),rs.getString("name"),rs.getString("image"),rs.getString("address"),rs.getFloat("rating"));
